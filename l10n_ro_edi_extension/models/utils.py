...
def make_efactura_request(
    ...
    Handles the response and returns a ``result`` dictionary.

    :param session: ``requests`` or ``requests.Session()`` object
    :param company: ``res.company`` object containing
        l10n_ro_edi_test_env, l10n_ro_edi_access_token
    :param endpoint: ``upload`` (for sending) | ``stareMesaj`` (status) |
        ``descarcare`` (download answer) | ``listaMesajeFactura`` (latest
        messages from efactura) | ``transformare`` (official PDF from efactura)
    :param params: Dictionary of query parameters
    :param data: XML data for ``upload`` request
    :return: ``{'error': str, ['timeout': True]}`` for errors, or
        ``{'content': response.content}`` from E-Factura
    """
    ...
    if endpoint == "transformare":
        standard = params.get("standard", "FACT1")
        novld = params.get("novld", "DA")
        url = (
            f"https://webservicesp.anaf.ro/prod/FCTEL/rest/transformare/"
            f"{standard}/{novld}"
        )
    ...
    try:
        ...
        return {
            "error": company.env._(
                "Timeout while sending to SPV. Use Synchronise to SPV to"
                " update the status."
            ),
            "timeout": True,
        }
...
def _request_ciusro_download_answer(company, key_download, session):
    """Make a "Download Answer" (GET/descarcare) request to the Romanian SPV.

    Processes the response by opening the received zip file and returns a
    dictionary containing:

    - the original invoice and/or the failing response from a bad request /
      unaccepted XML answer from the SPV
    ...
    :return: - ``{'error': str}`` if there has been an error during the
        request or parsing of the data
        - {
            'signature': {
                'attachment_raw': ``str``,
                'key_signature': ``str``,
    ...
    try:
        # The ZIP will contain two files, one with the electronic signature
        # (containing 'semnatura' in the filename), and the other with the
        # original invoice, the requested invoice or the identified errors.
...
def _request_ciusro_synchronize_invoices_pagination(company, session, nb_days=1):
    """Make a "Fetch Messages" (GET/listaMesajePaginatieFactura) request.

    After processing the response, if messages were indeed fetched, it will
    fetch the content of said messages. It uses the pagination system of the
    SPV to make sure all messages are fetched; by default only the first 500
    are shown, so for bigger companies this method needs to be paginated.

    Possible returns:
    Returns a dict with ``messages`` and ``errors`` keys. ``messages`` is a
    list of all messages obtained from SPV and ``errors`` is a list of all
    errors encountered during the pagination request or the fetching of the
    message content.
    ...
    :param company: ``res.company`` object
    :param session: ``requests.Session()`` object
    :param nb_days: (optional, default=1) ``int`` the number of days for
        which the request should be made, min=1, max=60
    :return: ``{'error': str}`` |
        ``{'sent_invoices_messages': [dict],
        'sent_invoices_refused_messages': [dict],
        'received_bills_messages': [dict]}``
    """
    ...
    def pagination_request(session, company, params):
        ...
        if "error" not in result:
            try:
                ...
                errors.append(
                    company.env._(
                        "The SPV data from page %s could not be parsed.", pagina
                    )
                )
        else:
            msg_content = json.loads(result["content"])
...
def _request_ciusro_synchronize_invoices(company, session, nb_days=1):
    """Process the messages from ``_request_ciusro_synchronize_invoices_pagination``.

    Fetches the content of each message and separates them into three
    categories:

    - Accepted sent invoices messages (accepted status of an invoice sent to
      SPV)
    - Refused sent invoices messages (refused status of an invoice sent to
      SPV)
    - Received bills messages (received status of an invoice from SPV)
    - Errors during the pagination request or the fetching of the message
      content
    ...
    representing a message. ``sent_invoices_messages`` contains all messages
    validating an invoice, ``sent_invoices_refused_messages`` contains all
    messages refusing an invoice, and ``received_bills_messages`` contains
    all messages representing received bills.
    """
...
def _request_ciusro_xml_to_pdf(company, xml_data, inv_type, validate_xml):
    """Make a 'transformare' request to get the official PDF of an invoice.

    :param company: ``res.company`` object
    :param xml_data: String of XML data to be sent
    :param inv_type: String, invoice type, either 'FACT1' or 'CN' depending
        on the move_type of the invoice
    :param validate_xml: String, either 'DA' or 'NU', depending on whether
        the XML needs validation
