...
class AccountMove(models.Model):
    ...
    def _make_payments_inv(self, date, company_ids):
        ...
        invoices =self.env['account.move'].search([('move_type','in',['out_invoice', 'in_invoice', 'out_refund', 'in_refund']),('state', 'in', ['posted']),('payment_state','in',['not_paid','partial']),('date','<',date),('company_id', 'in', company_ids)])
        journal = self.env["account.journal"].search([('name','=','payment_invoice')],limit=1)
        ...
        for invoice in invoices[:100]:
            ...
            try:
                ...
                payment = self.env["account.payment.register"].with_context(active_model="account.move", active_ids=[invoice.id]).create(payment_data)._create_payments()
