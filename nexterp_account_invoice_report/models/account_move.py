...
class AccountMove(models.Model):
    ...
    def _compute_tax_totals(self):
        ...
        if self.company_id.print_show_refunds and self.move_type in [
            "out_refund",
            "in_refund",
        ]:
            for move in self:
                    ...
                    tax_totals.update(
                        {
                            "formatted_amount_total": formatLang(
                                self.env,
                                (-1 * tax_totals["amount_total"]),
                                currency_obj=currency,
                            ),
                            "formatted_amount_untaxed": formatLang(
                                self.env,
                                (-1 * tax_totals["amount_untaxed"]),
                                currency_obj=currency,
                            ),
                        }
                    )
                        ...
                        tax_subtotal.update(
                            {
                                "formatted_amount": formatLang(
                                    self.env,
                                    (-1 * tax_subtotal["amount"]),
                                    currency_obj=currency,
                                ),
                            }
                        )

                    groups_by_subtotal_taxes = tax_totals["groups_by_subtotal"][
                        _("Untaxed Amount")
                    ]
                        ...
                        subtotal_tax.update(
                            {
                                "formatted_tax_group_amount": formatLang(
                                    self.env,
                                    (-1 * subtotal_tax["tax_group_amount"]),
                                    currency_obj=currency,
                                ),
                                "formatted_tax_group_base_amount": formatLang(
                                    self.env,
                                    (-1 * subtotal_tax["tax_group_base_amount"]),
                                    currency_obj=currency,
                                ),
                            }
                        )
